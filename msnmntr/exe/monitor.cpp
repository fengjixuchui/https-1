...
#define MSN_MESSENGER_FILE_PATH L"%ProgramFiles%\\Google\\Chrome\\Application\\chrome.exe"
//#define MSN_MESSENGER_FILE_PATH_WOW64 L"%ProgramFiles(x86)%\\Windows Live\\Messenger\\msnmsgr.exe"
#define MSN_MESSENGER_FILE_PATH_WOW64 L"%ProgramFiles(x86)%\\Google\\Chrome\\Application\\chrome.exe"
...
DWORD MonitorAppOpenMonitorDevice(OUT HANDLE* monitorDevice)
/*++
Routine Description:
...
BOOL MonitorAppCloseMonitorDevice(IN HANDLE monitorDevice)
/*++
Routine Description:
...
--*/
{
   ...
   result = FwpmCalloutDeleteByKey(engineHandle, &MSN_MONITOR_FLOW_ESTABLISHED_CALLOUT_V4);
   if (NO_ERROR != result)
   {
      goto abort;
   }
   ...
   result = FwpmCalloutDeleteByKey(engineHandle, &MSN_MONITOR_STREAM_CALLOUT_V4);
   if (NO_ERROR != result)
   {
      goto abort;
   }
...
DWORD MonitorAppDisableMonitoring(IN    HANDLE            monitorDevice)
/*++
Routine Description:
...
--*/
{
   ...
   monitorSubLayer.weight = 0;// We don't really mind what the order of invocation is.   
...
DWORD MonitorAppGetMSNMessengerApplicationId(__in const wchar_t* fileName, __out FWP_BYTE_BLOB** appId)
{
   DWORD result = NO_ERROR;
...
DWORD MonitorAppDoMonitoring()
{
   ...
   if (NO_ERROR != result)
   {
      result = MonitorAppGetMSNMessengerApplicationId(MSN_MESSENGER_FILE_PATH_WOW64, &applicationId);
   }
#endif
   ...
   result = MonitorAppAddFilters(engineHandle, applicationId);
   if (NO_ERROR != result)
   {
      goto cleanup;
   }
...
DWORD MonitorAppProcessArguments(__in int argc, __in_ecount(argc) PCSTR argv[])
{
