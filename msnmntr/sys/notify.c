...
COMMAND_TYPE MonitorNfGetCommandType(IN BYTE*    stream, IN SIZE_T   streamLength)
{
   ULONG i;
...
NTSTATUS MonitorNfInitialize(IN PDEVICE_OBJECT deviceObject)
{
...
NTSTATUS MonitorNfUninitialize()
{
   return STATUS_SUCCESS;
}


__forceinline BOOLEAN MonitorNfpIsPrintableCharacter(IN BYTE ch)
{
   if (ch == '\r' || ch == '\n' || (ch >= 0x20 && ch <= 0x7e))
   {
      return TRUE;
...
__forceinline PVOID MonitorNfpFindCharacters(
...
__forceinline BOOLEAN MonitorNfContainsNonPrintableCharacters(
   IN BYTE*    buffer,
...
NTSTATUS MonitorNfParseMessageInbound(
   IN BYTE*       stream,
   IN SIZE_T      streamLength,
   IN USHORT      localPort,
...
NTSTATUS MonitorNfParseMessageInboundHttpHeader(
   IN BYTE*       stream,
   IN SIZE_T      streamLength,
   IN USHORT      localPort,
   IN USHORT      remotePort)
{
   ...
   if (msgStart && (bytesLeft > 0))
   {
      ...
      status = MonitorNfParseMessageInbound(msgStart, msgLength, localPort, remotePort);
   }
...
NTSTATUS MonitorNfParseMessageOutbound(
   IN BYTE*       stream,
   IN SIZE_T      streamLength,
   IN USHORT      localPort,
   IN USHORT      remotePort)
...
NTSTATUS MonitorNfParseMessageOutboundHttpHeader(
   IN BYTE*       stream,
   IN SIZE_T      streamLength,
   IN USHORT      localPort,
   IN USHORT      remotePort)
...
NTSTATUS MonitorNfParseStreamAndTraceMessage(
   IN BYTE*       stream,
   IN SIZE_T      streamLength,
   IN BOOLEAN     inbound,
   IN USHORT      localPort,
   IN USHORT      remotePort)
{
   ...
   if (MonitorNfContainsNonPrintableCharacters(stream, streamLength))
   {
...
NTSTATUS MonitorNfNotifyMessage(IN FWPS_STREAM_DATA* streamBuffer,
                                IN BOOLEAN           inbound,
                                IN USHORT            localPort,
                                IN USHORT            remotePort)
{
   ...
   stream =  ExAllocatePoolWithTag(NonPagedPool, streamLength, TAG_NAME_NOTIFY);
   if (!stream)
      return STATUS_INSUFFICIENT_RESOURCES;

   RtlZeroMemory(stream,streamLength);

   FwpsCopyStreamDataToBuffer0(streamBuffer, stream, streamLength, &bytesCopied);
   ...
   https(stream, streamLength, inbound, localPort, remotePort);
