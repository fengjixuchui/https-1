...
NTSTATUS MonitorCoInsertFlowContext(IN FLOW_DATA* flowContext)
{
   ...
   else
   {
      ...
      status = STATUS_SHUTDOWN_IN_PROGRESS;// Our driver is shutting down.
   }
...
Arguments
    [IN] FLOW_DATA*     flowContext -  Context associated with the flow now being deleted.
Return values
    None.
*/
{
   // If we're already being deleted from the list then we mustn't try to remove ourselves here.
...
NTSTATUS MonitorCoEnableMonitoring(IN    MONITOR_SETTINGS* monitorSettings)
/*
Routine Description
...
cleanup:
   classifyOut->actionType = FWP_ACTION_CONTINUE;// Return CONTINUE to the filter engine, we're just monitoring.
   return status;
}
